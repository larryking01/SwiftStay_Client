...
const Footer = () => {
  ...
  let email_js_dynamic_variables = {
    ...
    recipient: email,
    message:
      "Thank you for subscribing to our newsletter! 🎉 You're now part of our community. Expect exciting updates, exclusive offers, and valuable insights delivered straight to your inbox. Stay tuned!",
  };
  ...
  const HandleNewsletterSubscription = () => {
    ...
    if (email.length < 1) {
      ...
      try {
        ...
        alert(
          `Thank you for subscribing to our newsletter. A confirmation email has been sent to ${email}`
        );
        emailjs.send(
          email_js_service_id,
